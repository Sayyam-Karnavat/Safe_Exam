...
@app.route("/get_crash_exam_index_answers/<student_id>", methods=['GET'])
def get_crashed_exam_details(student_id):
    try:
        # Pass the captured student_id to the method
        resume_index, question_answer_data = blockchain_obj.check_unleft_exam(student_id=str(student_id).strip())
